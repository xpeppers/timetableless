...
exports.RegistrationRepository = function RegistrationRepository(logError = console.error) {
  ...
  this.addPersonToNotify = (trainNumber, timeSlot, recipient) => registrationFor(trainNumber, timeSlot)
    .then((registration) => {
      registration.peopleToNotify = [...registration.peopleToNotify, recipient]
      return this.create(registration)
    })

  this.exists = (trainNumber, timeSlot) => registrationFor(trainNumber, timeSlot).then(registration => registration != null)
